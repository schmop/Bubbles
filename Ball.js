...
export default class Ball {
	...
	get otherBalls() { return this.game.balls; }
	...
	handleMouse() {
		if (this.clicked) {
			if (this.otherBalls.every(ball => !ball.boundToMouse))  {
				this.boundToMouse = true;
			}
		}
		if (!this.game.mouse.down) {
			this.boundToMouse = false;
		}
		if (this.boundToMouse) {
			let oldPos = this.pos;
			this.pos = this.game.mouse.pos.clone();
			this.vel = this.vel.scale(0.5).add(this.pos.sub(oldPos).scale(0.5));
		}
	}
